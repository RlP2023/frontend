...
const toFormData = (ship: Response) => {
    return {
        an_name: ship.an_name,
        result: ship.result,
        class_an: ship.class_an,
        animal_description: ship.animal_description,
        photo_animal: ship.photo_animal,
    }
}
const emptyShip: Response = {
    an_name: '',
    result: '',
    class_an: '',
    animal_description:'',
...
const  OrderPage: FC = () => {
    ...
    const deleteFromCart = async (ship_id: number) => {
        try {
            await axios(`http://127.0.0.1:8000/links/delete_animal_record/${ship_id}/`, {
                method: "DELETE",
                headers: {
                    'authorization': session_id
                }
    ...
    <Container >
        ...
        {(data) && data.records.map((ship: Ship)  => {
                        return (<Col style={{marginInline:'auto', width:"95%"}}>
                        <Ship_Card_Cart
                                    ship_id={ship.record_id}
                                    name={ship.rec_name}
                                    description={ship.description}
                                    photo_data={ship.photo_record}
