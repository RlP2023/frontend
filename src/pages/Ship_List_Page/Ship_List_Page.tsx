    ...
    return (
        ...
        />
            <Row>
                ...
                <Row style={{ display:'flex',width:"95%",marginInline:'auto'}}>
                    <div className ="box">
                        {respons.data.map((ship) => (
                            is_authenticated ?
                            <div>
                                <Ship_Card_with_Cart key={ship.record_id.toString()}
                            ...
                            <Ship_Card key={ship.record_id.toString()}
                            record_id={ship.record_id}
                            rec_name={ship.rec_name}
                            description={ship.description}
                            photo_record={ship.photo_record}
                    ...
                    <Table striped bordered hover variant="white">
                        ...
                        {respons.data.map((ship) => (
                            <tr>
                                <td>{ship.rec_name}</td>
                                <td>{ship.units}</td>
                                <td>{ship.env_measur}</td>
                                <td><Button onClick={() => {deleteShip(ship.record_id)}}>Удалить</Button></td>
                                <td><Button onClick={() => {editShip(ship.record_id)}}>Редактировать</Button></td>
                            </tr>
                        ))}
                        </tbody>
                    </Table>
