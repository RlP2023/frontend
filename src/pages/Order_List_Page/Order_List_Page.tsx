                ...
                <Table striped bordered hover variant="white" style={{width:"95%",marginInline:'auto'}}>
                        ...
                        {response.map((order) => (
                            <tr>
                                ...
                                <td>{order.animal_description ? 'Да' : "Нет"}</td>
                                <td><Button href={`/animals/${order.animal_id}`}>Просмотр</Button></td>
                                {order.status == "Active" && 
                                <td><Button onClick={() => {aprove(order.animal_id)}}>Подтверждение</Button></td>}
                                {order.status == "Active" && 
                                <td><Button onClick={() => {decline(order.animal_id)}}>Отклонение</Button></td> 
                                }
                            </tr>
                        ))}
                        </tbody>
